...
用法：
    python3 tools/verify_offline.py            # 跑全部
    python3 tools/verify_offline.py -v         # 显示每个节点的日志
    python3 tools/verify_offline.py --only vision,chassis
...
def check_vision(r: Runner) -> Result:
    out = subprocess.run([sys.executable, str(WS / "tools" / "make_test_image.py"), "--check"],
